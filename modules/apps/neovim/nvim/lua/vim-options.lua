...
-- 2色を t:0..1 で混ぜる (ステータスバーの装飾ブロックと同じ「薄め色」計算)
...
-- accent を白側に40%寄せた薄い色 (statusline Normal の左端装飾と同じ色)
local accent_pale = blend(mc.accent, "#ffffff", 0.4)
...
vim.api.nvim_create_autocmd("ColorScheme", {
    ...
    callback = function()
        ...
        -- 赤系は matugen の error ロールを参照する (他箇所のハードコード重複を解消)
        ...
        --   ロゴ/起動メッセージ = accent_pale (statusline Normal の装飾と同じ薄い黄色)，
        --   アイコン = accent，メニュー文字 = text，キー割当 = tertiary，Quit = 赤系
        -- snacks は行別指定が効かない場合に基底グループで描くため、基底も上書きする
        hl(0, "SnacksDashboardHeader", { fg = accent_pale, bold = true })
        for i = 1, 6 do
            hl(0, "SnacksDashboardHeader" .. i, { fg = accent_pale, bold = true })
        end
        ...
        hl(0, "SnacksDashboardFooter", { fg = accent_pale })
