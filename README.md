...
* 🗔 **[GlazeWM キーバインド・設定詳細](docs/glazewm.md)**: Windows環境におけるタイル型ウィンドウ操作，Zebar連携，一時停止モードの詳細．
...
* **ウィンドウマネージャー (`i3wm` / `GlazeWM`)**: Linux (i3wm) と Windows (GlazeWM) の両環境でゴールドとダークな色調で美しく統一されたタイル型ウィンドウ環境．
...
* 🗔 **[GlazeWM Keybindings & Configurations](docs/glazewm.md)**: Details on Windows tiling window management, Zebar status bar integration, and configuration bypass via Pause mode.
...
* **Window Manager (`i3wm` / `GlazeWM`)**: Custom Premium Gold (`#ffc20d`) / Dark Theme.
