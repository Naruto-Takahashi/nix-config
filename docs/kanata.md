...
左右の `Alt` キーを長押ししながらアルファベットを押すことで、i3wm の強力なウィンドウ操作機能を `Alt` キー単体の修飾で発動させることができます（裏側で自動的に `Super (Windowsキー)` と組み合わせた信号に変換しています）。

* **Alt単押し**: IMEのオン/オフ切り替え（以下参照）。
* **Alt長押し (Hold) + 他のキー**: Altレイヤー（i3wm操作層）へ移行します。

### 🪟 Alt長押し中のキーマッピング一覧

| キー | 実際に送信される信号 | 発動する i3wm アクション |
| :---: | :--- | :--- |
