...
- `\` 継続の複数行コマンドは改行・インデントごと記録され、`Tab`/`Enter` での呼び出し時に縦に並んだ元の形で復元される。リスト内では1行に畳んで表示 (改行は見た目だけ空白に変換し `^J` 表記が出ないようにしている)、選択すると下部の空白区切りの先に縦の形のまま表示される (`preview.strategy = "auto"`)。プレビューが伸びても結果リストは最低限の行数を保つ
- レイアウトは検索バー上・結果下 (`invert = true`)、fzf 風の枠線付き (`style = "full"`)。セクション間はダッシュの罫線ではなく空白1行で区切られる。キーヘルプは非表示
